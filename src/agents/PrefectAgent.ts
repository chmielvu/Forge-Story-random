...
- \`sceneContext\` (JSON): location (id), timeOfDay, playersPresent[], facultyPresent[], sceneFlags[], otherPrefects[].
  ...
  async think(scene: FilteredSceneContext, ledger: YandereLedger): Promise<PrefectDecision> {
    ...
    try {
      ...
      if (decision.stateDelta?.favorScoreDelta) {
        this.updateFavorScore(decision.stateDelta.favorScoreDelta);
      }
